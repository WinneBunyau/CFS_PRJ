...
//@WebServlet("/AddCropProduceServlet")
public class AddCropProduceServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, javax.servlet.http.HttpServletResponse response) 
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                 "INSERT INTO crop_produce (crop_id, quantity, crop_produce_date, crop_storage_location) VALUES (?, ?, ?, ?)")) {
            ps.setInt(1, cropId);
        ...
        if(session != null && session.getAttribute("adminId") != null) {
            response.sendRedirect("adminCropProduceManagement.jsp?success=Crop produce added");
        } else {
            response.sendRedirect("farmerCropProduceManagement.jsp?success=Crop produce added");
        }
    }
}
